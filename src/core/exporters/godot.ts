...
/**
 * Godot 4 exporter: the zip IS a complete Godot project — project.godot with
 * run/main_scene pointing at a generated scene.tscn (one PlaneMesh per layer,
 * plus camera, sun, fog and glow). Extract → Import in the Project Manager →
 * press Play. Mirrors the three.js runtime's coordinate math (same y-up
 * right-handed space, so no handedness flip needed).
 */
export function godotFiles(pack: AssetPack): Record<string, Uint8Array> {
  ...
  return {
    ...baseFiles(pack),
    "project.godot": text(projectGodot(scene.name)),
    "scene.tscn": text(out.join("\n") + "\n"),
    "README-godot.txt": text(GODOT_README),
  };
}

/** Minimal project.godot that opens straight into the generated scene. */
function projectGodot(name: string): string {
  const safe = name.replace(/["\\\r\n]+/g, " ").trim() || "PixelStage Scene";
  return `; Engine configuration file.
; It's best edited using the editor UI and not directly,
; since the parameters that go here are not all that obvious.
;
; Generated by PixelStage — a complete Godot 4 project.
...
This zip IS a complete Godot 4 project — no manual setup.

Open & play
-----------
1. Extract this zip anywhere.
2. In the Godot Project Manager, click "Import" and pick project.godot
   from the extracted folder.
3. Press F5 (Play) — the scene runs exactly as framed in PixelStage:
   layers, camera, sun, fog and glow included.
